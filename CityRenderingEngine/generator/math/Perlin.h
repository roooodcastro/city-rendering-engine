...
public:
    ...
    static float getCityBlockDensity(float posX, float posY) {
        ...
        return getNoiseAt((abs(posX) / noiseScale) + offset, noiseHeight, (abs(posY) / noiseScale) + offset);
    }
...
protected:
    ...
    // TODO: Create variables to hold the functions parameters and load them from a file on the first time a function is called

    static bool parametersLoaded;
};

int Perlin::p[] = {};
bool Perlin::parametersLoaded = false;
const int Perlin::permutations[] = { 151,160,137,91,90,15,
